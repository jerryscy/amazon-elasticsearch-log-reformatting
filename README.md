...
3. Go to Amazon Elasticsearch Kibana, select **Dev Tools** in the left navigation
4. Run below command to see if the index has been created

  ```text
GET /_cat/indices/access-log-12345678-*?v&s=index
```

After 5 mins, if you cannot see an index like

### Trouble shooting 
1. Go to **Cloudwatch**
2. select **Log groups** with a name **/aws/kinesisfirehose/access-log-stream-12345678**
3. Search all event for last 30m. You should see error like this
```text
{
...
2. Select **elasticsearch-failed/** (it will show up if there is any error. The file path is yyyy/mm/dd)
3. Download the error log. You can see similar error.

Key take away: Leverage Cloudwatch and S3 elasticsearch-failed for troubleshooting
...
2. Click the IAM role in the Permissions session
3. Copy the Role ARN, like arn:aws:iam::{account ID}:role/service-role/KinesisFirehoseServiceRole-access-log-st-us-east-1-xxxxxxxxxxxxx
4. Login into Amazon Elastisearch Kibana
5. Select in **Security** under **Management**
6. Select **all_access** in **Roles** 
7. Select **Manage mapping** in **Mapped users**
8. Add another backend role
9. Paste the Role ARN in steps 3 into backend role, like arn:aws:iam::{account ID}:role/service-role/KinesisFirehoseServiceRole-access-log-st-us-east-1-xxxxxxxxxxxxx
10. **Map**
...
2. In Amazon Elasticsearch, run delete command to delete the demo data
```shell
Delete /access-log-12345678-*
```
...
1. Remote to the instance which installed the kinesis agent
2. Run below command to create the log generator script 
```shell
...
3. Generate some access log
```shell
/tmp/loggenerator.sh
```

4. Go to Amazon Elasticsearch Kibana, select **Stack Management** in the left navigation
5. Go to **Index Patterns** and **Create index pattern**
6. In Index pattern name type **access-log-12345678\***
7. **Next step**
...
2. Click **Edit**
3. Select `Enabled` in **Source record transformation**
  * Lambda function: access-log-12345678-log-converter
  * Buffer size: 1 MiB
  * Buffer interval: 60 seconds
  * **Save**
4. Go to **Amazon Elasticsearch Kibana Dev Tools** and execute below command to delete the existing configuration
```text
Delete /access-log-12345678*
```
5. In the **EC2** instance, run `/tmp/loggenerator.sh` to generate new log.
7. Wait for 1 min
8. Go to **Amazon Elasticsearch Kibana Dev Tools** and execute below command to delete the existing configuration
```text
GET /_cat/indices/access-log-12345678*?v&s=index
```
...
9. Go to **Amazon Elasticsearch Kibana Stack Management**
10. Select **access-log-12345678\*** and remove index pattern by clicking the trash icon in right top corner
11. **Delete**
12. **Create index pattern** with index pattern name type **access-log-12345678\***
13. **Next step**
14. Select `timestamp` under **Time field**
15. **Create index pattern** 

![Expected result](./docs/images/aes-index-pattern-afterlogprasing.png)

16. Go To Amazon Elasticsearch Kibana, select **Discover** in the left navigation
17. Select **access-log-12345678\*** in the index pattern
![Expected result](./docs/images/aes-discover-access-log-12345678.png)

> Key takeaway: By rewriting the `access_log_parser` in the code, you can transform your data into an Amazon Elasticsearch readable forma
